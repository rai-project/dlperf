...
func TestUnmarshal(t *testing.T) {
	bench := &Suite{}
	err := json.Unmarshal([]byte(testBenchmarkData), bench)
...
func TestFilterByNameBenchmarks(t *testing.T) {
	bench := &Suite{}
	err := json.Unmarshal([]byte(testBenchmarkData), bench)
	assert.NoError(t, err)
	assert.NotEmpty(t, bench)
	benches, err := bench.FilterByName("SGEMM/128.*")
	assert.NoError(t, err)
...
func TestFilterBenchmarks(t *testing.T) {
	...
	assert.NoError(t, err)
	assert.NotEmpty(t, bench)
	benches, err := bench.Filter(Benchmark{
